...
import { PageBreak } from "./Titles";
import "./components.css"
const eventsJSON = await fetchIPFSData();
const numberOfEvents = await eventsJSON.events.length;

const TicketsOwned = () => {
  const [ownedTickets, setOwnedTickets] = useState(Array.from({ length: numberOfEvents}, v => Array.from({ length: 6 }, v => null)));
      ...
      { totalTickets > 0 
        ? <>
          <h7 style={{ color: "white" }}>
            You have {totalTickets} ticket
            {totalTickets > 1 ? "s" : ""} in total!
          </h7>
          <Accordion class="accordion" style={{ fontWeight: "bold" }}>
            {ownedTickets.map((e, id) => ( 
              <>
              <PageBreak height="30px" />
              <Accordion.Item eventKey={id} style={{ backgroundColor: "black" }}>
